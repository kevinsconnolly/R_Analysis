...
# Use tidyverses dplyr library which is the basis of transforming R data.
library(tidyverse)
library(ggplot2)
...
# multiple linear regression model we need to obtain our statistical metrics using the summary() function.
...
# determine if suspension coil's pound-per-inch (PPI)results are statistically different from the mean population results of 1,500 pounds per inch.
...
t.test((sample_table$PSI),(sample_table2$PSI)) #means of two samples compared